  ...
  head: () => ({
    meta: [
      { title: "Your ATS Scan Results — Career Copilot AI" },
      {
        name: "description",
        content:
          "See your ATS match score, missing keywords, and unlock the AI-rewritten resume that gets you interviews.",
      },
...
function ScoreDial({ score, unlocked }: { score: number; unlocked: boolean }) {
  const size = 240;
  const stroke = 18;
  ...
  const color = unlocked
    ...
    : score < 50
      ...
      : "oklch(0.78 0.17 75)"; // amber/yellow
    ...
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <AnimatePresence mode="wait">
          ...
          >
            <div className="font-display text-6xl font-bold tracking-tight" style={{ color }}>
              {score}
...
function ResultsPage() {
  ...
  const statusLabel = unlocked ? "Optimized" : score < 50 ? "Critical" : "Needs Work";
    ...
    <div className="min-h-screen bg-gradient-to-b from-secondary/40 via-background to-background">
      ...
      <main className="mx-auto max-w-6xl px-4 pb-24 pt-10 sm:px-6 sm:pt-14">
        ...
        <div className="mb-8 flex flex-col items-start justify-between gap-3 sm:mb-10 sm:flex-row sm:items-end">
          ...
          <span className={`rounded-full px-3 py-1.5 text-xs font-semibold uppercase tracking-wider ${statusTone}`}>
            {statusLabel}
          </span>
        </div>

        {/* Free view: score + missing keywords */}
        <section className="grid gap-6 lg:grid-cols-5">
          <div className="rounded-3xl border border-border/70 bg-card p-8 shadow-[var(--shadow-card)] lg:col-span-2">
            <div className="flex flex-col items-center">
              <ScoreDial score={score} unlocked={unlocked} />
              <p className="mt-6 max-w-xs text-center text-sm text-muted-foreground">
                {unlocked
                  ? "Excellent. Your resume is now hitting the top 5% of applicants for this role."
                  : "You're missing critical keywords the ATS scans for. Recruiters may never see this application."}
              </p>
            </div>
          </div>

          <div className="rounded-3xl border border-border/70 bg-card p-6 sm:p-8 shadow-[var(--shadow-card)] lg:col-span-3">
            <div className="mb-5 flex items-center justify-between">
              <div>
                <h2 className="font-display text-xl font-semibold tracking-tight">Missing Keywords</h2>
        ...
        {/* Paywall section */}
        <section className="relative mt-10 overflow-hidden rounded-3xl border border-border/70 bg-card shadow-[var(--shadow-elevated)]">
          <div className="border-b border-border/60 bg-gradient-to-r from-secondary/60 to-transparent px-6 py-5 sm:px-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-display text-xl font-semibold tracking-tight">Critical Fixes &amp; AI Rewrite</h2>
                <p className="text-sm text-muted-foreground">
                  The exact edits to push your score from {unlocked ? "65" : "65"} → 95.
                </p>
              </div>
            </div>
          </div>
              ...
              {[
                ...
                <div key={i} className="rounded-2xl border border-border/70 bg-background p-4 sm:p-5">
                  <div className="mb-1.5 flex items-center gap-2">
                    ...
                    <div className="font-display text-sm font-semibold tracking-tight">{item.h}</div>
                  </div>
        ...
        {/* Downloads */}
        <section className="mt-8 rounded-3xl border border-border/70 bg-card p-6 sm:p-8 shadow-[var(--shadow-card)]">
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              <h2 className="font-display text-lg font-semibold tracking-tight">Export your optimized resume</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                {unlocked
